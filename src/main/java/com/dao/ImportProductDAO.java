...
import com.resultset.ImportProductResultSet;
import com.rowmapper.ImportProductRowMapper;
...
public class ImportProductDAO implements BaseDAO<ImportProduct> {
    ...
    public List<ImportProduct> findByProduct(Long product){
        String sql= "select ip.*,p.*, pl.name as productline_name from import_product ip left join product p on p.id=ip.id_product join productline pl on p.id_productline = pl.id where p.id=?";
